...
public class RevocableJwtTokenStore implements TokenStore {
  ...
  @Autowired
  public RevocableJwtTokenStore(
      JwtAccessTokenConverter jwtAccessTokenConverter, TokenRepository repository
  ) {
  ...
  public void storeRefreshToken(
    ...
    Instant expirationTime = ((DefaultExpiringOAuth2RefreshToken) refreshToken)
        .getExpiration().toInstant();

    int offsetInSeconds = ZoneId.systemDefault()
        .getRules().getOffset(Instant.now()).getTotalSeconds();
  ...
  public OAuth2RefreshToken readRefreshToken(String tokenValue) {
    DefaultExpiringOAuth2RefreshToken refreshToken =
        (DefaultExpiringOAuth2RefreshToken) plainStore.readRefreshToken(tokenValue);
